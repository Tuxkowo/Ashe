require(['../ashe', '../ashe.modifiers'], function (Ashe, AsheModifiers) {
	/**
	 * Generals Ashe tests.
	 */
	describe('General Tests', function() {
		...
		it('Ashe should be defined as Ashe', function() {
	...
	describe("Render loops", function() {
		...
		beforeEach(function() {
			tpl =
				"<ul> \
					{% for user in users %} \
					<li>{{user.name}}, {{user.age}} - {{outer}}</li> \
					{% else %} \
					<li>No users yet</li> \
					{% endfor %} \
				</ul>";
		...
		it("Simple non-empty loop", function() {
			...
			expect(out).htmlToBeEqual("<ul><li>Thomas Mann, 26 - Member</li><li>Jassy Brook, 23 - Member</li></ul>");
		});
